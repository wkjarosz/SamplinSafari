...
public:
    Hammersley(unsigned dimensions = 2, unsigned numSamples = 64) :
        BaseHalton(dimensions - 1), m_numSamples(numSamples), m_inv(1.0f / m_numSamples)
    {
    }
    ~Hammersley() override {}
    ...
    {
        BaseHalton::setSeed(seed);
        m_scramble1 = seed ? m_rand.nextUInt() : 0;
    }

    std::string name() const override { return std::string("Hammersley (") + BaseHalton::name() + ")"; }
