...
{
    if (m_resX == 0) m_resX = 1;
    if (m_resY == 0) m_resY = 1;
    ...
    if (m_randomize)
    {
        ...
        for (unsigned i = 0; i < m_resX; i++)
            for (unsigned j = m_resY-1; j >= 1; j--)
            {
                ...
                std::swap(m_samples[0][j * m_resX + i],
                            m_samples[0][k * m_resX + i]);
            }
        ...
        for (unsigned j = 0; j < m_resY; j++)
            for (unsigned i = m_resX-1; i >= 1; i--)
            {
                ...
                std::swap(m_samples[1][j * m_resX + i],
                            m_samples[1][j * m_resX + k]);
            }
    }
}
...
{
    return tfm::format("MultiJittered In-Place");
}
