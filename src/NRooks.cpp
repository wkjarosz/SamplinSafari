...
NRooks::reset()
{
    if (m_numSamples == 0) m_numSamples = 1;
...
void
NRooks::sample(float r[], unsigned i)
{
    if (i >= m_numSamples)
        i = 0;
    ...
    for (unsigned d = 0; d < dimensions(); d++)
        r[d] = (m_permutations[d][i] +
                0.5f + jitter * (m_rand.nextFloat() - 0.5f)) * m_scale;
}
...
void NRooksInPlace::sample(float r[], unsigned i)
{
    ...
    float jitter = m_maxJit * m_randomize;
    for (unsigned d = 0; d < dimensions(); d++)
        r[d] = (permute(i, m_numSamples, m_scrambles[d]) +
                0.5f + jitter * (m_rand.nextFloat() - 0.5f)) / m_numSamples;
}
